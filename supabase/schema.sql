...
-- ---------- leaderboard ----------
-- The RLS policy above only lets each student see their own `users` row —
-- correct for privacy. This SECURITY DEFINER function is the one sanctioned
-- way past that: it returns only the fields safe to rank publicly (name,
-- XP, level, rank, streak), never email or anything else.
